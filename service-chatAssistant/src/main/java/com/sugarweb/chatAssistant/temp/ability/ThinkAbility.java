...
public class ThinkAbility {
    ...
    public ThinkAbility(ExecutorService executor) {
    ...
    public void streamThink(List<BaseMsg> msgList, StreamingResponseHandler<AiMessage> extHandler) {
        String agentId = "1";
        String memoryId = "1";
        ...
        StreamingResponseHandler<AiMessage> baseHandler = new StreamingResponseHandler<>() {
            ...
            public void onComplete(Response<AiMessage> response) {
                ...
                extHandler.onComplete(response);
            }

            @Override
            public void onError(Throwable error) {
                extHandler.onError(error);
            }

            @Override
            public void onNext(String token) {
                extHandler.onNext(token);
            }

        };
        //todo 有待争议；测试看看是异步方法还是同步方法
