...
public class DatasetMemoryComponent {
    ...
    public DatasetMemoryComponent(Dataset dataset) {
        if (dataset == null) {
            throw new IllegalArgumentException("kbInfo is null");
        }
        this.embeddingStore = MilvusEmbeddingStoreFactory.create(dataset);
        Model embeddingModel = dataset.getEmbeddingModel();
