...
import com.sugarweb.digitalHuman.infra.agent.input.blbl.BlblMsgPrompt;
...
public class StreamThinkComponent {
    ...
    private final KbMemoryComponent kbMemoryComponent;
    ...
    @Builder
    public StreamThinkComponent(EnvironmentContext environmentContext, InputContainer inputContainer, KbMemoryComponent kbMemoryComponent, ChatMemoryComponent chatMemoryComponent, List<StreamListener> listeners) {
        this.executor = environmentContext.getExecutor();
        this.inputContainer = inputContainer;
        this.kbMemoryComponent = kbMemoryComponent;
        ...
        SceneInfo sceneInfo = environmentContext.getSceneInfo();
        // 设置聊天主题提示词
        this.topicSystemPrompt = sceneInfo.getSystemPrompt();
    }
    ...
    public void answer(long thinkId) {
        ...
        //获取相关召回文档
        String documents = "无";
        if(kbMemoryComponent != null) {
            String retrievalSegment = kbMemoryComponent.getRetrievalSegment(question);
        ...
        // 历史消息
        List<ChatMsg> chatMsgs = chatMemoryComponent.lastChatMessage(blblUser.getBlblUid(), 10);
